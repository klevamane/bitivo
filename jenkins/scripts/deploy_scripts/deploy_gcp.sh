...
run_deployment_script() {
    ...
    chmod +x jenkins/scripts/api_deploy_scripts/deploy_to_gcp.sh
    ./jenkins/scripts/api_deploy_scripts/deploy_to_gcp.sh
}
...
clone_infrastructure_repository() {
    ...
    echo $SERVICE_ACCOUNT | base64 -d  > /home/circleci/activo-infra/ansible/account.json
}
